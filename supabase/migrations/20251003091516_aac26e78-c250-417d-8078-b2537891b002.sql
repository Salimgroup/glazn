-- =====================================================
-- PHASE 1: CRITICAL SECURITY FIXES (v2 - Fixed)
-- =====================================================
...
-- Create new restricted policy: Users can see ALL their own data
CREATE POLICY "profile_own_full_access"
ON public.profiles FOR SELECT
USING (auth.uid() = id);

-- Create new public policy: Others can only see non-sensitive data (NO financial data)
-- This policy will filter out total_spent and total_earnings columns
...
CREATE POLICY "activity_feed_restricted_access"
...
-- Add length constraints to prevent abuse (skip if exists)
DO $$ BEGIN
